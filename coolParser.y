...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    %type <expression>  expression_log_op
    %type <expression>  expression_arith_op
    %type <expression>  expression_const
    ...
    | '{' expression_semicolon_list '}'                        { $$ = block($2); }
    ...
    | expression '@' TYPEID '.' OBJECTID '(' expression_comma_list ')'  { $$ = static_dispatch($1, $3, $5, $7); }
    | expression '.' OBJECTID '(' ')'                          { $$ = dispatch($1, $3, nil_Expressions()); }
    | expression '.' OBJECTID '(' expression_comma_list ')'    { $$ = dispatch($1, $3, $5); }
    ...
    | expression_comma_list ',' expression  { $$ = append_Expressions($1, single_Expressions($3)); }
    ;

    expression_semicolon_list
    : expression ';'                            { $$ = single_Expressions($1); }
    | expression_semicolon_list expression ';'  { $$ = append_Expressions($1, single_Expressions($2)); }
